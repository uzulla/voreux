...
## 15. AI エージェントがシナリオを書くとき

AI エージェント（Claude、Copilot、Cursor など）にシナリオ作成を依頼する場合は、追加の行動指針があります。

詳細は [docs/agent-behavior-for-scenario-authoring.md](./agent-behavior-for-scenario-authoring.md) を参照してください。

要点:
- エージェントは自分の能力（画像理解、ブラウザ操作など）を正直に報告すべき
- DOM やピクセル数値だけでなく、人間の目に見える変化かどうかを常に判断する
- 段階的に作業し、各ステップで「何が視覚的に変わったか」を説明する
- サンプルが human-visible な変化を生み出せない場合は、無理にテストを書かず報告する
