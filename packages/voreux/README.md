...
- **録画対応**: テスト実行の様子を動画出力
- **Recorder scaffold tool**: Chrome DevTools Recorder JSON から Draft scenario の土台を生成できる
...
## Chrome DevTools Recorder から Draft scaffold を作る

Voreux には、Chrome DevTools Recorder が export した JSON を元に、
**ハッピーパス E2E の初期骨格となる Draft scenario** を生成する PoC ツールがあります。

これは **不具合再現レポート生成ツールではなく**、
**正常系フローを E2E 化するための scaffold generator** です。
...
# または
node ./dist/devtools-recorder-cli.js recording.json > scaffold.draft.test.ts
```

現状のPoC方針:
- 標準入力またはファイルから Recorder JSON を受け取る
